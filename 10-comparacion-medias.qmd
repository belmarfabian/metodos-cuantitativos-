...
| Pregunta | Situación | Prueba |
|----------|-----------|--------|
| ¿Mi muestra difiere de un valor conocido? | Un grupo vs valor teórico | t de una muestra |
| ¿Difieren dos grupos independientes? | Hombres vs mujeres, tratados vs control | t de dos muestras |
| ¿Cambió algo antes y después? | Mismas personas medidas dos veces | t pareada |
| ¿Difieren tres o más grupos? | Regiones, niveles educativos, etc. | ANOVA |
...
**El p-value:**
Como p < 0.05, concluimos que existe una diferencia estadísticamente significativa en los ingresos promedio entre hombres y mujeres.

```{r plot-genero, echo=TRUE, fig.cap="Comparación de ingresos por género"}
# Visualización
...
### ¿Por qué es diferente a la prueba de dos muestras?
...
## ANOVA: Comparando más de dos grupos
...
### Ejemplo: Satisfacción por orientación política
...
```{r anova-plot, echo=TRUE, fig.cap="Satisfacción con la democracia según orientación política"}
...
| Prueba paramétrica | Alternativa no paramétrica | En R |
|-------------------|---------------------------|------|
...
| Situación | Prueba | En R |
|-----------|--------|------|
| Una muestra vs valor conocido | t de una muestra | `t.test(x, mu = valor)` |
| Dos grupos independientes | t de dos muestras | `t.test(x, y)` |
| Antes/después (mismas personas) | t pareada | `t.test(x, y, paired = TRUE)` |
| Tres o más grupos | ANOVA | `aov(y ~ grupo)` |
...
a) ¿Cuál es la pregunta de investigación?
b) Realiza la prueba t con `t.test()`
c) Interpreta el p-value: ¿hay diferencia significativa?
d) Interpreta el intervalo de confianza
e) Escribe una oración resumiendo el resultado
...
b) ¿Cuál grupo tiene mayor participación cívica?
c) ¿La diferencia es estadísticamente significativa?
d) Crea un boxplot comparando ambos grupos
e) ¿Qué tan grande es la diferencia en términos prácticos?

**3. ¿Cuándo usar prueba pareada?**

Indica si deberías usar una prueba t pareada o de muestras independientes:

a) Comparar notas de estudiantes antes y después de un curso
b) Comparar satisfacción laboral entre empresas públicas y privadas
c) Comparar presión arterial de pacientes antes y después de un tratamiento
d) Comparar ingresos entre graduados de dos universidades diferentes
e) Comparar actitud hacia la inmigración en las mismas personas en 2020 y 2024
...
c) ¿Cuál es el cambio promedio?
d) ¿El cambio es estadísticamente significativo?
e) ¿Podemos concluir que el documental *causó* el cambio? ¿Por qué?
...
c) Crea un boxplot de satisfacción por región
d) ¿Cuál región tiene mayor satisfacción? ¿Cuál menor?
e) Si el ANOVA es significativo, ¿qué prueba adicional necesitas para saber cuáles regiones difieren específicamente?
...
b) ¿Qué variable quieres comparar?
c) ¿Qué prueba usarías y por qué?
d) ¿Qué resultado esperarías encontrar?
e) Si la diferencia fuera significativa, ¿qué conclusiones podrías sacar? ¿Cuáles no?
