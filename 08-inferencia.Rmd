...
Pero solo podemos encuestar una **muestra** de cientos o miles de personas, no los millones que componen la población.

::: {.objetivos}
**Inferencia estadística**: El proceso de usar datos de una muestra para hacer afirmaciones sobre una población, cuantificando la incertidumbre asociada.
:::
...
Los **parámetros** $(\mu, p, \sigma^2)$ son valores fijos (pero desconocidos) de la población.

Los **estadísticos** $(\bar{x}, \hat{p}, s^2)$ son calculados a partir de la muestra y **varían** entre muestras.

Usamos estadísticos para **estimar** parámetros.
:::

::: {.ejemplo}
**Caso: Apoyo a reforma tributaria**

- **Población**: 15 millones de votantes chilenos
- **Parámetro de interés**: $p$ = proporción que apoya la reforma (desconocida)
- **Muestra**: 1,200 votantes encuestados
- **Estadístico observado**: $\hat{p} = 0.42$ (42% de la muestra apoya)

**Pregunta inferencial**: ¿Qué podemos decir sobre $p$ (desconocido) a partir de $\hat{p} = 0.42$ (conocido)?
:::
...
::: {.nivel}
**Observación clave**: Aunque la población de ingresos es altamente asimétrica, la distribución de $\bar{X}$ es aproximadamente Normal. Esto es el Teorema del Límite Central en acción.
:::
...
::: {.ejemplo}
**Encuesta electoral: Distribución de $\hat{p}$**

Si el verdadero apoyo es $p = 0.45$ y encuestamos $n = 1000$:
...
::: {.importante}
**Error estándar vs desviación estándar**^[CONFUSIÓN COMÚN: El error estándar ($\text{SE}$) mide qué tan precisa es nuestra estimación de la media. La desviación estándar ($s$) mide qué tan dispersos están los datos individuales. Son conceptos diferentes: $\text{SE} = s/\sqrt{n}$ siempre es menor que $s$ (excepto cuando $n=1$).]

- **Desviación estándar ($s$)**: Mide variabilidad en los **datos**
- **Error estándar ($\text{SE}$)**: Mide variabilidad del **estadístico** (precisión de la estimación)
- $\text{SE}$ disminuye con $n$; $s$ NO (permanece constante en promedio)
:::

::: {.ejemplo}
**Caso: Ingreso familiar en encuesta CASEN**
...
::: {.importante}
**Interpretación correcta de IC 95%:**

"Si repitiéramos este proceso de muestreo muchas veces, aproximadamente 95% de los intervalos construidos contendrían el verdadero valor de $\mu$."

**NO significa:** "Hay 95% de probabilidad de que $\mu$ esté en este intervalo específico."

(La diferencia es sutil pero importante: $\mu$ es fijo; el intervalo es lo que varía entre muestras.)
:::

::: {.ejemplo}
**Intervalo de confianza para ingreso promedio**
...
::: {.ejemplo}
**Estimar ingreso promedio con precisión de ±$20 mil**
...
- Agresti, A. & Finlay, B. (2009). *Statistical Methods for the Social Sciences*. Capítulo 5: Statistical Inference: Estimation.
- Llaudet, E. & Imai, K. (2022). *Data Analysis for Social Science*. Capítulo 7: Uncertainty.
- Diez, D. et al. (2019). *OpenIntro Statistics*. Capítulo 4: Foundations for inference. [Acceso libre](https://www.openintro.org/book/os/)
