...
|----------|-------------|--------------|
| **Correlación** | Fuerza y dirección de relación lineal (-1 a 1) | `cor(x, y)` |
| **Regresión** | Modela $Y = \beta_0 + \beta_1 X + \epsilon$ | `lm(y ~ x)` |
| **Intercepto** ($\beta_0$) | Valor de $Y$ cuando $X = 0$ | `coef(modelo)[1]` |
| **Pendiente** ($\beta_1$) | Cambio en $Y$ por unidad de $X$ | `coef(modelo)[2]` |
| **$R^2$** | Proporción de variabilidad explicada | `summary(modelo)$r.squared` |
...
e) ¿Qué limitaciones tiene la correlación para establecer causalidad?
...
c) ¿El coeficiente de gasto es estadísticamente significativo?
...
c) ¿Tiene sentido el intercepto en este contexto? ¿Por qué?
d) Predice participación para una comuna con 12 años de educación promedio
e) ¿Qué sucede si la comuna tiene 8 años de educación promedio?
...
b) ¿Los residuos parecen distribuirse normalmente? (histograma o Q-Q plot)
c) Grafica residuos vs. valores ajustados. ¿Hay patrones preocupantes?
...
b) ¿Cuál variable explica más variabilidad?
c) ¿Los coeficientes son significativos en ambos modelos?
d) Si tuvieras que elegir solo una variable predictora, ¿cuál elegirías?

**6. Proyecto de regresión**

Con datos de tu interés:

a) Formula una hipótesis sobre la relación entre dos variables
b) Estima un modelo de regresión simple
c) Interpreta todos los componentes del output de `summary()`
d) Verifica supuestos básicos
e) Crea un gráfico de dispersión con la línea de regresión
f) Escribe un párrafo reportando los resultados
g) Discute las limitaciones de tu análisis para establecer causalidad
