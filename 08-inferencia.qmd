...
Es importante distinguir entre **error estándar** y **desviación estándar**. La desviación estándar ($s$) mide variabilidad en los **datos**, mientras que el error estándar ($\text{SE}$) mide variabilidad del **estadístico** (precisión de la estimación). El $\text{SE}$ disminuye con $n$; $s$ NO (permanece constante en promedio).[^se-vs-sd]

[^se-vs-sd]: El error estándar mide qué tan precisa es nuestra estimación de la media. La desviación estándar mide qué tan dispersos están los datos individuales. $\text{SE} = s/\sqrt{n}$ siempre es menor que $s$ (excepto cuando $n=1$).
...
::: {.callout-warning}
## Error común: Interpretación incorrecta del IC
...
::: {.callout-important}
## Mensaje clave
El "95%" NO se refiere a una muestra individual, sino al **procedimiento**. Si lo repitiéramos infinitas veces, 95% de los intervalos contendrían $\mu$. Para cualquier muestra específica, o contiene $\mu$ o no (pero no sabemos cuál).
:::
...
|----------|-------------|
| **Parámetros** ($\mu$, $p$) | Valores poblacionales fijos (desconocidos) |
| **Estadísticos** ($\bar{x}$, $\hat{p}$) | Valores muestrales variables (observados) |
| **Error estándar** | SD de la distribución muestral: $\text{SE} = s/\sqrt{n}$ |
...
c) ¿Cuántas personas necesitarías encuestar para reducir el SE a la mitad del original?
...
a) Calcula la proporción muestral
b) Construye IC 95% manualmente
c) Verifica con `prop.test()` en R
d) Interpreta el intervalo en lenguaje no técnico
...
a) Calcula media y desviación estándar muestral
b) Construye IC 95% manualmente
c) Verifica con `t.test()` en R
d) Construye IC 90% y 99%. ¿Qué observas sobre el ancho de los intervalos?

**4. Tamaño muestral**

Quieres estimar la proporción de votantes que apoya una ley, con margen de error de ±2 puntos porcentuales y 95% de confianza.

a) ¿Qué tamaño muestral necesitas? (usa fórmula conservadora con p=0.5)
b) Si solo tienes presupuesto para 600 encuestas, ¿cuál será tu margen de error?
c) ¿Cómo cambia el tamaño muestral necesario si quieres 99% de confianza?

**5. Simulación de intervalos de confianza**

```{r eval=FALSE}
# Simula 200 muestras de una población conocida
set.seed(2024)
mu_real <- 100
sigma_real <- 15
n <- 50
...
a) Ejecuta la simulación y calcula: ¿cuántos ICs contienen μ = 100?
b) ¿El resultado es cercano al 95% esperado?
c) Grafica los primeros 50 ICs (como en el capítulo)
d) Repite con nivel de confianza 90%. ¿Cuántos contienen μ ahora?
...
a) ¿Qué tamaño muestral aproximado tuvo la encuesta?
b) ¿Cuál es el IC 95%?
c) ¿Podemos concluir que "la mayoría desaprueba"? Justifica
d) Si otra encuesta con n=2000 reporta 49% de aprobación, ¿son los resultados "contradictorios"?
